...
menuhit:=prog(cover:Cover, M:chan of Mouse, ms:Mouse, menu:Menu, b:int) of int{
	n:=len menu.lab;
	if(n==0)
		become menuspin(cover.cover, M, ms, -1);
	...
	lfree(cover, cover.layer[0].seq);
	if(ms.but&~b)
		item=-1;
	become menuspin(cover.cover, M, ms, item);
};
